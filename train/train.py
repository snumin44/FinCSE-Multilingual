...
from src.data_loader import Dataset_CSV, Dataset_TXT, DataCollator
...
def train(encoder, train_dataloader, test_dataset, optimizer, scheduler, scaler, args):
    ...
    for epoch_i in range(args.epochs):           
        ...
        for step, batch in tqdm(enumerate(train_dataloader), total=len(train_dataloader)):
            ...
            if args.amp:
                train_loss = encoder(input_ids, attention_mask)
            ...
            if args.eval_strategy == 'steps' and (step+1) % args.eval_step == 0:
                ...
                top1_acc, top5_acc = results['top1_accuracy'], results['top5_accuracy']
                print(f'Epoch:{epoch_c}, Top1 Acc:{round(top1_acc*100, 2)} (%), Top5 Acc:{round(top5_acc*100, 2)} (%)')
    ...
    print(f"Training Time: {training_time}, Average Training Loss: {avg_train_loss}, Best Top1 Acc: {round(best_score*100, 2)} (%)")

def main(args):
    init_logging()
    seed_everything(args)
    
    LOGGER.info('*** Train Multilingual SimCSE ***')
    ...
    valid_dataset = Dataset_CSV.load_dataset(args.valid_data)
