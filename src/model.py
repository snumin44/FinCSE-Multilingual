...
class SimCSE(nn.Module):
    ...
    def __init__(self, args):
        ...
        if 'xlm-roberta' in args.model:
            ...
            self.tokenizer = XLMRobertaTokenizerFast.from_pretrained(args.tokenizer)
        else:   
    ...
    @autocast()
    def forward(self, input_ids, attention_mask): # token_type_ids 제거
        ...
        outputs =  self.encoder(input_ids=input_ids, attention_mask=attention_mask) # token_type_ids=token_type_ids 제거
    ...
    def get_embeddings(self, input_ids, attention_mask): # token_type_ids 제거
        outputs =  self.encoder(input_ids=input_ids, attention_mask=attention_mask) # token_type_ids=token_type_ids 제거
        pooler_output = self.pooler(attention_mask, outputs)
